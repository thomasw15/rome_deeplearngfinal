...
def compute_v(
    ...
    print("Computing right vector (v0)")

    # Tokenize target into list of int token IDs
    target_ids = tok(request["target_new"][0]["str"], return_tensors="pt").to("cuda")[
        "input_ids"
    ][0]

    # Compile list of rewriting and KL x/y pairs
    rewriting_prompts, kl_prompts = [
        context.format(request["prompt"][0]) + tok.decode(target_ids[:-1])
        for context in context_templates
    ], ["{} is a"]
    all_prompts = rewriting_prompts + kl_prompts

    input_tok = tok(
        [prompt.format(request["subject"][0]) for prompt in all_prompts],
        return_tensors="pt",
        padding=True,
    ).to("cuda")
    ...
    lookup_idxs = [
        find_fact_lookup_idx(
            prompt, request["subject"][0], tok, hparams.fact_token, verbose=(i == 0)
        )
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        print(
            ...
            f"avg prob of [{request['target_new'][0]['str']}] "
    ...
    cur_input0, cur_output0 = get_module_input_output_at_word(
        ...
        tok,
        layer,
        context_template=request["prompt"][0],
    ...
    # Tokenize target into list of int token IDs
    target_ids = tok(request["target_new"][1]["str"], return_tensors="pt").to("cuda")[
        "input_ids"
    ][0]

    # Compile list of rewriting and KL x/y pairs
    rewriting_prompts, kl_prompts = [
        context.format(request["prompt"][1]) + tok.decode(target_ids[:-1])
        for context in context_templates
    ], ["{} is a"]
    all_prompts = rewriting_prompts + kl_prompts

    input_tok = tok(
        [prompt.format(request["subject"][1]) for prompt in all_prompts],
        return_tensors="pt",
        padding=True,
    ).to("cuda")
    ...
    lookup_idxs = [
        find_fact_lookup_idx(
            prompt, request["subject"][1], tok, hparams.fact_token, verbose=(i == 0)
        )
        for i, prompt in enumerate(all_prompts)
    ]
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        print(
            f"loss {np.round(loss.item(), 3)} = {np.round(nll_loss.item(), 3)} + {np.round(kl_loss.item(), 3)} + {np.round(weight_decay.item(), 3)} "
            f"avg prob of [{request['target_new'][1]['str']}] "
            f"{torch.exp(-nll_loss_each).mean().item()}"
        )
        if loss < 5e-2:
            break
    ...
    cur_input1, cur_output1 = get_module_input_output_at_word(
        ...
        tok,
        layer,
        context_template=request["prompt"][1],
